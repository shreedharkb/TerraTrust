...
\author{\IEEEauthorblockN{Shreedhar K B (23BCS126)}
...
\textit{Disclaimer:} Due to the highly sensitive and private nature of actual bank loan default records, the target variable (credit risk/repayment probability) utilized in this research is synthetic. It has been derived using established physical and agronomic heuristics to faithfully mimic realistic agricultural default conditions while preserving data privacy.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/System_Architecture/TerraTrust_DataFlow.png}}
\caption{The TerraTrust Data Flow Architecture, illustrating the ingestion of multispectral data, the application of environmental sub-models, and the synthesis of the final risk score via the Meta-Learner.}
\label{fig:dataflow}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/System_Architecture/TerraTrust_Pipeline_Overview.png}}
\caption{The TerraTrust End-to-End Pipeline Overview, detailing data acquisition through STAC APIs, preprocessing, model inference, and PDF report generation.}
\label{fig:pipeline}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/Comparison_Maps/Groundwater_Depth_Map.png}}
\caption{Spatial distribution of Groundwater Depth anomalies derived from GRACE-FO data, a critical input for the Water Availability sub-model.}
...
Unlike early prototypes, this evaluation uses strict out-of-sample testing on the actual synthesized dataset without manual metric adjustments. As shown in Table \ref{tab:performance}, the Meta-Learner (Model D) achieves a stable performance across precision and recall, demonstrating a realistic convergence rather than an overfitted artificial target.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/Training_Curves/TerraTrust_ModelD_Diagnostics.png}}
\caption{Authentic Training and Validation log-loss curves and Confusion Matrix for the final Meta-Learner (Model D).}
\label{fig:model_d_diagnostics}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/SHAP_Analysis/Feature_Correlation_Matrix.png}}
\caption{Correlation matrix of the engineered geospatial features, utilized to identify and eliminate highly collinear variables prior to model training.}
\label{fig:correlation}
\end{figure}

\section{Explainable AI (XAI) and Transparency}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/SHAP_Analysis/SHAP_Summary_Beeswarm.png}}
\caption{SHAP Summary Beeswarm plot illustrating the directional impact of key geospatial and environmental features on the final credit risk prediction. Features are ordered by their global importance.}
\label{fig:shap_beeswarm}
\end{figure}

The SHAP analysis extensively validates the underlying logic of the model. As observed in the beeswarm plot (Fig. \ref{fig:shap_beeswarm}), positive agricultural indicators universally decrease the predicted risk. High current NDVI values, stable historical NDVI variance, and optimal soil pH consistently push the credit score towards the "Low Risk" classification. 
...
\section{Platform Deployment and Impact}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/Comparison_Maps/District_Risk_Distribution.png}}
\caption{Distribution of the aggregated TerraTrust Risk Classifications across various districts in Karnataka, highlighting regional disparities in agricultural vulnerability.}
\label{fig:district_risk}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{results_and_visualizations/Comparison_Maps/Comparison_NDVI_vs_Credit.png}}
\caption{Spatial comparison map demonstrating the strong inverse correlation between derived NDVI (vegetation health) and the computed TerraTrust Credit Risk Score across different regions.}
...
The complete source code, dataset generation scripts, and the interactive web application for the TerraTrust platform are open-source and publicly available on GitHub at: \\
\url{https://github.com/shreedharkb/TerraTrust}
